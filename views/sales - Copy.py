    ...
    def print_receipt(self):
        ...
        try:
            ...
            if target_printer:
                ...
                # Items header - compact format with grid lines
                txt_lines.append(('BODY', '+' + '-' * 4 + '+' + '-' * 18 + '+' + '-' * 8 + '+' + '-' * 9 + '+'))
                txt_lines.append(('HEADER', "| Qty| Item Name        | Price  | Total   |"))
                txt_lines.append(('BODY', '+' + '-' * 4 + '+' + '-' * 18 + '+' + '-' * 8 + '+' + '-' * 9 + '+'))
                
                # Items - compact format with grid lines (Excel-like)
                for it in sd.get('items', []):
                    n = (it.get('name') or '')
                    q = int(it.get('quantity', 0))
                    prc = float(it.get('price', 0.0))
                    amt = q * prc
                    # Truncate long names to fit in grid cell (16 chars max)
                    name_display = n[:16] if len(n) > 16 else n
                    # Format with grid borders
                    line = f"|{q:>3} | {name_display:<16} |{prc:>7.2f} |{amt:>8.2f} |"
...
class SalesWidget(QWidget):
    ...
    def _handle_ctrl_r(self):
        """Handle Ctrl+R shortcut - in refund mode, zero out all items except selected one"""
        try:
            if getattr(self, 'is_refund_mode', False):
                ...
                if selected_row >= 0 and selected_row < len(self.current_cart):
                    # Zero out all items EXCEPT the selected one
                    print(f"[SHORTCUT] Ctrl+R: Zeroing all items except row {selected_row}")
                    for i, item in enumerate(self.current_cart):
                        if i != selected_row:
                            item['quantity'] = 0
                    
                    # Remove zero-quantity items
                    self.current_cart = [item for item in self.current_cart if item['quantity'] > 0]
                    
                    self.update_cart_table()
                    self.update_totals()
                    print(f"[SHORTCUT] Ctrl+R: Kept only item at row {selected_row}")
                else:
                    print("[SHORTCUT] Ctrl+R: No item selected in refund mode")
            else:
                print("[SHORTCUT] Ctrl+R: Focusing refund invoice input")
    ...
    def create_cart_section(self, parent_layout):
        ...
        try:
            ...
            try:
                ...
                pass
            sc.activated.connect(self._handle_ctrl_r)
            self._sc_ctrl_r = sc
        except Exception:
            pass
    ...
    def eventFilter(self, obj, event):
        ...
        try:
            ...
            # 2a. Down arrow from search input - if no suggestions, go to cart
            if search_input and obj is search_input and key == Qt.Key_Down:
                if not sugg_list or sugg_list.count() == 0:
                    # No suggestions, go directly to cart if it has items
                    if cart_tbl and cart_tbl.rowCount() > 0:
                        cart_tbl.setFocus()
                        cart_tbl.selectRow(0)
                        print("[DEBUG] Navigation: Search -> Cart (no suggestions)")
                        return True
                # If suggestions exist, let the code above handle it
                return False
    ...
    def keyPressEvent(self, event):
        ...
        try:
            # Let refund invoice input handle all keys normally
            if self.focusWidget() is getattr(self, 'refund_invoice_input', None):
                # Don't block any keys for refund invoice input - let it handle everything
                return
        except Exception:
            pass
    ...
    def add_product_to_cart(self, product):
        ...
        # Get current stock level from database to ensure accuracy
        try:
            from pos_app.models.database import Product
            current_product = self.controller.session.query(Product).filter(Product.id == getattr(product, 'id', None)).first()
            if current_product:
                stock_level = int(getattr(current_product, 'stock_level', 0))
            else:
        ...
        for item in self.current_cart:
            if item['id'] == getattr(product, 'id', None):
                # Stock check - use database stock level
                ...
                self.update_cart_table()
                self.update_totals()
                # Fix 6: Don't focus cart table - let search field keep focus
                return
        ...
        cart_item = {
            ...
            'quantity': 1,
            'stock_level': stock_level  # Use accurate stock level
        }
        ...
        self.update_cart_table()
        self.update_totals()
        # Fix 6: Don't focus cart table - let search field keep focus for rapid entry
        # The focus will be managed by _on_suggestion_selected or add_first_search_result
